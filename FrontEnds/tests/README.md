...
| `test-sprite-animations-dungeon-fisher.sh` | Shell script | SpriteAnimator tween animations: module structure (6 methods: idle/attack/hit/faint/stopIdle/destroy), idle two tweens (y bob + scale breathe, repeat:-1/yoyo:true), attack Promise (lunge, white flash, snap-back, resume idle), hit Promise (red tint, shake), faint Promise (angle 90, alpha, y drop), BattleScene integration (idle on create, attack/hit in execAttack, faint on death, fish-switch destroy+replace), TitleScene starter fish idle (portrait + landscape), FloorScene recruit idle (37 checks) |
...
bash tests/test-sprite-animations-dungeon-fisher.sh
```
