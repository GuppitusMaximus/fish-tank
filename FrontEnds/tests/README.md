...
| `test-dynamic-dungeon-sizing.sh` | Shell script | Dynamic dungeon container sizing: desktop @media (min-width:601px) sets height:auto/flex:1/min-height:0 on #dungeon, no aspect-ratio or max-height in desktop block, shared #tank/#arena/#sky/#dungeon rule intact, mobile max-width:600px still sets explicit height, body has display:flex + flex-direction:column (9 checks) |
...
| **Dynamic dungeon container sizing: desktop @media (min-width:601px) uses height:auto/flex:1/min-height:0 (viewport-filling); no aspect-ratio or max-height in desktop block; shared game container rule unmodified; mobile still uses fixed height; body flex layout required for flex:1 child** | \`test-dynamic-dungeon-sizing.sh\` |
