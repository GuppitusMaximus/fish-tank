...
| `test-dungeon-fisher-back-to-menu.sh` | Shell script | Persistent MENU button: UIOverlayScene creates [ MENU ] at top-left (4,3) with depth 1000, scrollFactor 0, interactive+hover+stroke, hidden on TitleScene/BootScene, visible on gameplay scenes, click stops all gameplay scenes and starts TitleScene, Continue works after return, version label intact, VERSION=0.10.0 in both version.js and package.json (33 checks) |
...
| **Persistent MENU button: UIOverlayScene [ MENU ] at top-left (depth 1000, scrollFactor 0, interactive+hover+stroke), hidden on TitleScene/BootScene, visible on all gameplay scenes (CharacterSelect/Floor/Battle/Shop/Camp/Victory/ZonePreview), click stops all gameplay scenes + starts TitleScene, Continue works after return, VERSION=0.10.0** | \`test-dungeon-fisher-back-to-menu.sh\` |
